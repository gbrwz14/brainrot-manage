...
def add_job(request: AddJobRequest, db: Session = Depends(get_db)):  
    """Adiciona um novo job_id à fila de servidores a serem scaneados"""  
    now = datetime.utcnow()  
      
    # Verifica se o job já existe  
    existing = db.query(Server).filter_by(job_id=request.job_id).first()  
    if existing:  
        return {"status": "already_exists", "job_id": request.job_id}  
      
    # Cria novo servidor na fila  
    server = Server(  
    ...
    return {"status": "added", "job_id": request.job_id, "id": server.id}  
